...
const QUIZ_SETS = [
  ...
  {
    id: "EQ-BASIC3",
    name: "สมการ ป.6 เข้า ม.1 สมการ เศษส่วน",
    total: 10,
    passingScore: 8,
    timeLimit: 30 * 60,
  },
  {
    id: "EQ-BASIC2",
    name: "สมการ ป.6 เข้า ม.1 สมการ วงเล็บ",
    total: 10,
    passingScore: 8,
    timeLimit: 30 * 60,
  },
  {
    id: "EQ-BASIC1",
    name: "สมการ ป.6 เข้า ม.1 สมการ ย้ายห่าง",
    total: 10,
    passingScore: 8,
    timeLimit: 30 * 60,
  },
...
function getSetFromUrl() {
  try {
    return new URLSearchParams(window.location.search).get("set") || null;
  } catch {
    return null;
  }
}
...
// ── SCREEN: SET SELECT (admin only) ───────────────────────
function SetSelectScreen({ onSelect, theme }) {
  const [search, setSearch] = useState("");
  const filtered = QUIZ_SETS.filter(
          ...
          {filtered.map((set) => (
            ...
            >
              <div>
                ...
                >
                  {set.id} · {set.total}ข้อ · {set.timeLimit / 60}นาที · ผ่าน{" "}
                  {set.passingScore}/{set.total}
                </div>
                <div
                  style={{
                    color: "#3a6a3a",
...
function LoginScreen({ set, onConfirm, onBack, isDirectLink, theme }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* ปุ่มย้อนกลับ — แสดงเฉพาะตอนไม่ได้เข้าจาก direct link */}
        {!isDirectLink && (
          <button
            onClick={onBack}
            style={{
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "16px" }}>
          ...
          <button
            ...
            style={{
              ...
              fontFamily: "'Cinzel',serif",
              fontSize: "13px",
              cursor: sid.trim() && !loading ? "pointer" : "not-allowed",
              whiteSpace: "nowrap",
            }}
          >
            {loading ? "..." : "ค้นหา"}
          ...
          >
            ...
            >
              ...
              <button
                ...
                style={{
                  background: "linear-gradient(135deg,#1a4a1a,#27ae60,#1a4a1a)",
                  border: "none",
                  borderRadius: "10px",
                  padding: "12px",
                  color: "#fff",
...
function QuizScreen({ set, student, questions, onFinish, theme }) {
  ...
  const finish = useCallback(
    (timeUp = false) => {
      ...
      const results = questions.map((q, qi) => {
        ...
        const si = answers[qi] ?? null;
        const oi = si !== null ? shuffled[si].origIndex : null;
        return {
          question: q,
          selectedOrigIndex: oi,
  ...
  const selNow = answers[current] ?? null;
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {student.nickname} · {set.id} · ข้อ{" "}
            <b style={{ color: tc }}>{current + 1}</b>/{questions.length}
          </span>
          <span
            style={{
              fontFamily: "'Courier New',monospace",
              ...
              fontWeight: 700,
              color:
                timeLeft < 60 ? "#e74c3c" : timeLeft < 180 ? "#e67e22" : tc,
              textShadow:
                timeLeft < 60 ? "0 0 10px rgba(231,76,60,.7)" : "none",
            }}
          >
            ⏱ {formatTime(timeLeft)}
          </span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "9px" }}>
          {shuffled.map((choice, si) => {
            ...
            return (
              <button
                key={si}
                onClick={() => setAnswers((a) => ({ ...a, [current]: si }))}
                style={{
              ...
              >
                <span
                  style={{
                    ...
                    borderRadius: "50%",
                    flexShrink: 0,
                    background: sel ? tc : `${tc}11`,
                    border: sel ? "none" : `1px solid ${tc}33`,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    fontSize: "12px",
...
function ResultScreen({ data, onRetry, onHome, isDirectLink, theme }) {
  ...
  // ── state เฉลย — แยกจาก state อื่นทั้งหมด ──
      ...
      try {
        ...
        for (const r of rareOK) {
          if (r.question.seriesId)
            await apiPost({
              action: "saveRareProgress",
              studentId: student.id,
              seriesId: r.question.seriesId,
    ...
    >
      <div
        style={{
          ...
          background:
            "linear-gradient(160deg,rgba(20,12,5,.97),rgba(38,22,8,.97))",
          border: `2px solid ${
            passed ? "rgba(39,174,96,.5)" : "rgba(231,76,60,.4)"
          }`,
          borderRadius: "16px",
          padding: "32px 28px",
          boxShadow: "0 20px 60px rgba(0,0,0,.8)",
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: "24px" }}>
          ...
          >
            {correct}
            <span style={{ fontSize: "26px", color: "#6b5a3e" }}>
              /{results.length}
            </span>
          </div>
          <div
        ...
        >
          {[
            ["✅ ถูก", `${correct} ข้อ`, "#27ae60"],
            ["❌ ผิด", `${results.length - correct} ข้อ`, "#e74c3c"],
            ["⏱ เวลา", formatTime(timeUsed), tc],
        ...
        {/* ── ปุ่มดูเฉลย — แยกออกจากปุ่มอื่นชัดเจน ── */}
        ...
        {showDetail && (
          ...
          >
            {results.map((r, i) => {
              const correctText =
                r.shuffledChoices.find((c) => c.origIndex === r.question.answer)
                  ?.text ?? "-";
              const selectedText =
                r.selectedOrigIndex !== null
                  ? r.shuffledChoices.find(
                      (c) => c.origIndex === r.selectedOrigIndex
                    )?.text ?? "-"
                  : "ไม่ได้ตอบ";
              return (
                <div
                  ...
                  style={{
                    background: r.isCorrect
                      ...
                      : "rgba(231,76,60,.07)",
                    border: `1px solid ${
                      r.isCorrect ? "rgba(39,174,96,.3)" : "rgba(231,76,60,.3)"
                    }`,
                    borderRadius: "10px",
                    padding: "14px",
                  }}
                >
                  ...
                  >
                    {r.isCorrect ? "✅" : "❌"} ข้อ {i + 1}
                    {r.question.isRare && (
                      <span style={{ color: "#9b59b6", marginLeft: "8px" }}>
                        ✦ หายาก
                      </span>
                    )}
                  </div>
                  ...
                  >
                    {r.isCorrect ? (
                      ...
                      <span>
                        คุณตอบ:{" "}
                        <span style={{ color: "#e74c3c" }}>{selectedText}</span>
                        {" · "}เฉลย:{" "}
                        <span style={{ color: "#27ae60", fontWeight: 600 }}>
                          {correctText}
                        </span>
        ...
        {/* ── ปุ่มด้านล่าง — แยกจากปุ่มเฉลยชัดเจน ── */}
        <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
          {/* ซ่อนปุ่มหน้าหลักถ้าเข้าจาก direct link เพราะจะกลับไปแค่ login */}
          {!isDirectLink && (
            <button
              type="button"
              onClick={onHome}
...
export default function App() {
    ...
    if (setId) {
      ...
      if (found) {
        ...
        apiGet({ action: "getConfig", setId }).then((d) => {
          if (d.config) setTheme(buildTheme(d.config));
        });
        setScreen("login");
  ...
  if (screen === "init")
    return <div style={{ minHeight: "100vh", background: theme.bgColor }} />;
